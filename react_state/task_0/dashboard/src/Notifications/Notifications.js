  ...
  render() {
    ...
    return (
      <div id='container' className={css(styles.container)}>
        <div className={`menuItem ${css(styles.menuItem)} ${css(styles.fadeBounceIn)}`}>
          Your notifications
        </div>
        {displayDrawer && (
            ...
            <ul className={css(styles.lists)}>
              {listNotifications.length === 0 ? (
                ...
                listNotifications.map(({ type, html, value, id }) => (
                  <NotificationItem key={id} type={type} html={html} value={value} markAsRead={this.markAsRead} className={`notification-item ${css(type === 'default' ? styles.defaultNotifications : styles.urgentNotifications)}`} />
                ))
              )}
            </ul>
          </div>
